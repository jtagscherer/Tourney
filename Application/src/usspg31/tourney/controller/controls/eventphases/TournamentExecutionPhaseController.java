...
public class TournamentExecutionPhaseController implements TournamentUser {
	...
	public void loadTournament(Tournament tournament) {
		...
		this.buttonEnterResult.disableProperty().bind(this.pairingView.selectedPairingProperty().isNull());
	}
	...
	@FXML private void onButtonStartRoundClicked(ActionEvent event) {
	...
	@FXML private void onButtonEnterResultClicked(ActionEvent event) {
		...
		roundFinishCheck:
			for (Pairing pairing : this.loadedTournament.getRounds().get(
					this.loadedTournament.getRounds().size() - 1).getPairings()) {
