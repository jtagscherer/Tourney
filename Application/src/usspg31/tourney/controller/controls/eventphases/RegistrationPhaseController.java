        ...
        if (selectedFile != null) {
            ...
            if (importedEvent != null) {
                ...
                if (!duplicateStartingNumbers) {
                    ...
                    for (Tournament importedTournament : importedEvent
                            .getTournaments()) {
                        for (Tournament loadedTournament : this.loadedEvent
                                .getTournaments()) {
                            if (importedTournament.getId().equals(
                                    loadedTournament.getId())) {
                                inner_loop: for (Player importedPlayer : importedTournament
                                        .getRegisteredPlayers()) {
                                    for (Player loadedPlayer : loadedTournament
                                            .getRegisteredPlayers()) {
                                        if (loadedPlayer.getId().equals(
                                                importedPlayer.getId())) {
                                            continue inner_loop;
                                        }
                                    }
                                    loadedTournament.getRegisteredPlayers()
                                            .add(importedPlayer);
                                }
                            }
                        }
