...
public class TournamentPhaseDialog extends VBox implements IModalDialogProvider<GamePhase, GamePhase> {
	...
	private static final Logger log = Logger.getLogger(TournamentPhaseDialog.class.getName());
	...
	@FXML private void initialize() {
		this.textFieldPlayTimeSeconds.numberValueProperty().addListener((ov, o, n) -> {
			if (n.intValue() >= 60) {
	...
	private void loadGamePhase(GamePhase gamePhase) {
		...
		this.textFieldCutoff.numberValueProperty().bindBidirectional(gamePhase.cutoffProperty());
		this.textFieldRoundCount.numberValueProperty().bindBidirectional(gamePhase.roundCountProperty());
		this.textFieldPlayTimeMinutes.setNumberValue((int) gamePhase.getRoundDuration().getSeconds() / 60);
		this.textFieldPlayTimeSeconds.setNumberValue((int) gamePhase.getRoundDuration().getSeconds() % 60);
		this.textFieldPlayersPerPairing.numberValueProperty().bindBidirectional(gamePhase.numberOfOpponentsProperty());
	...
	private void unloadGamePhase() {
		...
		this.textFieldCutoff.numberValueProperty().unbindBidirectional(this.loadedPhase.cutoffProperty());
		this.textFieldRoundCount.numberValueProperty().unbindBidirectional(this.loadedPhase.roundCountProperty());
		this.textFieldPlayersPerPairing.numberValueProperty().bindBidirectional(this.loadedPhase.numberOfOpponentsProperty());
